...
public class BlurTextView extends TextView {
    ...
    private void blur(Bitmap bkg) {
        ...
        Bitmap overlay = Bitmap.createBitmap((int) (getMeasuredWidth() / mSacleFactor),
                (int) (getMeasuredHeight() / mSacleFactor), Bitmap.Config.ARGB_8888);
        Canvas canvas = new Canvas(overlay);
        canvas.translate(-getLeft() / mSacleFactor, -getTop() / mSacleFactor);
        canvas.scale(1 / mSacleFactor, 1 / mSacleFactor);
        Paint paint = new Paint();
        paint.setFlags(Paint.FILTER_BITMAP_FLAG);
        canvas.drawBitmap(bkg, 0, 0, paint);

        overlay = fastblur(overlay, 4 / mSacleFactor, (int) mRadius);
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.JELLY_BEAN) {
            setBackground(new BitmapDrawable(getResources(), overlay));
        } else {
            setBackgroundDrawable(new BitmapDrawable(getResources(), overlay));
        }
